...
class RegisterAction(argparse.Action):
	def __call__(self, parser, namespace, values, option_string=None):
		...
		print "" #URL
		print "ziz-rg350.py --update" #Call for updating the list
...
class UpdateAction(argparse.Action):
	def __call__(self, parser, namespace, values, option_string=None):
		...
		if len(output) > 1:
			...
			for line in output:
				if line.startswith("<a href=puzzletube-rg350.opk"):
		...
		if len(output) > 1:
			...
			output = process.stdout.read().split('\n')
			for line in output:
				if line.startswith("<a href=Sparrow-C4A-Manager-rg350.opk"):
					#searching the filename itself
					parts = line.split('<a href=')
					parts = parts[1].split(' ')
					print "filename: " + parts[0] #NEEDED!
		...
		if len(output) > 1:
			...
			output = process.stdout.read().split('\n')
			for line in output:
				if line.startswith("<a href=glutexto-rg350.opk"):
					#searching the filename itself
					parts = line.split('<a href=')
					parts = parts[1].split(' ')
					print "filename: " + parts[0] #NEEDED!
		...
		if len(output) > 1:
			...
			output = process.stdout.read().split('\n')
			for line in output:
				if line.startswith("<a href=snowman-rg350.opk"):
					#searching the filename itself
					parts = line.split('<a href=')
					parts = parts[1].split(' ')
					print "filename: " + parts[0] #NEEDED!
		...
		if len(output) > 1:
			...
			output = process.stdout.read().split('\n')
			for line in output:
				if line.startswith("<li><a href=hase-rg350.opk"):
					#searching the filename itself
		...
		if len(output) > 1:
			...
			output = process.stdout.read().split('\n')
			for line in output:
				if line.startswith("<a href=sissi-rg350.opk"):
					#searching the filename itself
					parts = line.split('<a href=')
					parts = parts[1].split(' ')
					print "filename: " + parts[0] #NEEDED!
		...
		if len(output) > 1:
			...
			output = process.stdout.read().split('\n')
			for line in output:
				if line.startswith("<a href=meteoroid3d-rg350.opk"):
					#searching the filename itself
					parts = line.split('<a href=')
					parts = parts[1].split(' ')
					print "filename: " + parts[0] #NEEDED!
